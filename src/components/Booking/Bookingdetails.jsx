...
const BookingDetails = () => {
  const { state: apt } = useLocation();
  const navigate = useNavigate();

  if (!apt) return <h3>Apartment not found.</h3>;
  ...
  return (
    <div className="booking-container">
      <div className="hotel-header">
        <h2 className="hotel-name">{apt.name}</h2>
        <p className="hotel-location">{apt.location}</p>
        <div className="star-rating1">
          {"★".repeat(Math.round(rating)) +
            "☆".repeat(5 - Math.round(rating))}
        </div>
        <div className="hotel-price">${apt.price}/night</div>
      </div>
