...
const Results = () => {
    ...
    const load = async () => {
      try {
        ...
        const filtered = res.data.filter(
          (apt) =>
            apt.city.toLowerCase() === city.toLowerCase() &&
            Number(apt.guests) >= guests
        );
  ...
  if (loading) return <p className="loading-text">Loading apartments...</p>;
  if (!apartments.length) return <NoHotelsFound />;

  return (
    <div className="result-container">
      <h2 className="result-title">
        Apartments in {city} — {apartments.length} found
      </h2>
      <ul className="hotel-grid">
        {apartments.map((apt) => {
          const rv = getRandomReview();
          return (
            <li
              key={apt._id}
            ...
            >
              <img
                src={`http://localhost:5000${apt.images[0]}`}
                alt={apt.name}
                className="hotel-image"
              />
              <div className="hotel-info">
                <h3 className="hotel-name">{apt.name}</h3>
                <p className="hotel-price">${apt.price}/night</p>
                <div className="review-box">
                  <span className="star-rating">{generateStars(rv.rating)}</span>
                  <span className="rating-box">{rv.rating}</span>
                  <span className="rating-text">{rv.text}</span>
                  <span className="review-count">
                    {Math.floor(Math.random() * 300 + 20)} reviews
                  </span>
                </div>

                {/* Optional: show selected check-in/out */}
                {checkIn && checkOut && (
                ...
                <button className="view-place-btn">View</button>
              </div>
            </li>
          );
        })}
